...
public class Register extends AppCompatActivity {
    ...
    Button mRegister;
    EditText mFullaname,mEmail,mPassword,mConfirmpassword;
...
//if the user is already registered and the device pref:company is not empty(ie has been set) send the user to the login screen
        if (firebaseAuth.getCurrentUser() != null && !sp.getString("companynam", "").equals("") && !sp.getString("branchnam", "").equals("") && !sp.getString("requestreset", "").equals("yes")) {
            startActivity(new Intent(getApplicationContext(), Login1.class));
            finish();
        } else if(firebaseAuth.getCurrentUser() == null || sp.getString("requestreset", "").equals("yes")){
